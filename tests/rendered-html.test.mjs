...
test("builds a self-contained GitHub Pages dashboard", async () => {
  ...
  assert.match(bundle, /How repositories reach this list/);
...
test("keeps execution outside the browser and validates discovery provenance", async () => {
  ...
  assert.equal(review.confirmationStatus, "researcher_confirmation_required");
  ...
  assert.equal(review.decisions.filter((decision) => decision.status === "integrated").length, 3);
