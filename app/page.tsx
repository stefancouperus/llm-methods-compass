...
import { ARCHITECTURE_REGISTER, MODEL_DISCOVERY, MODEL_REGISTRY } from "./model-registry";
...
function deriveMethodRoutes(form: FormState): MethodRoute[] {
  ...
  if (has("interpretation")) {
    routes.push({
      ...
      recommendation: "Write the researcher's reading first; use a local instruction model only for bounded alternative readings, counterevidence and questions to pursue.",
      reason: "Interpretation depends on context, standpoint and scholarly warrant. Model suggestions may broaden attention, but they do not validate themselves and should remain linked to exact source evidence.",
      tests: ["Compare researcher-first with model-assisted conditions.", "Record useful additions, invented support, flattening of ambiguity and anchoring effects.", "Retain disagreement and indeterminacy when they are substantively meaningful."],
      caution: "No model is treated as an interpretive authority or default winner.",
      references: [{ label: "Kommers et al.", url: "https://doi.org/10.3389/frai.2026.1753041" }, { label: "Ngo et al.", url: "https://arxiv.org/abs/2602.18352" }],
    });
  }
...
function deriveValidationPlan(form: FormState): ValidationPlan {
  ...
  if (has("interpretation") || form.downstreamUse === "interpretive") {
    strategies.push({
      ...
      actions: ["Record a researcher-first reading before model exposure.", "Require exact source support, counterevidence and alternative readings.", "Audit anchoring, invented context, anachronism and flattening of ambiguity."],
      limitation: "A fluent explanation or agreement among models cannot establish the uniquely correct historical interpretation.",
      references: [{ label: "Kommers et al. — computational hermeneutics", url: "https://doi.org/10.3389/frai.2026.1753041" }, { label: "Röttger et al. — plural judgments", url: "https://doi.org/10.18653/v1/2022.naacl-main.13" }],
    });
  }
...
## Automated discovery watchlist

- Updated: ${MODEL_DISCOVERY.generatedAt}
- Status: metadata leads only; not validated recommendations
      ...
      <main className="main-shell">
        ...
        {view === "recommendation" ? (
          <div className="content recommendation-view">
            ...
            <section className="discovery-watchlist">
              <div className="section-title-row"><div><p className="eyebrow">Recently found model repositories</p><h2>Models that may be worth inspecting</h2><p>Updated {displayDate(MODEL_DISCOVERY.generatedAt)} from the public Hugging Face catalogue. These are search results, not recommendations or test results.</p></div><StatusPill tone="warn">Catalogue information only</StatusPill></div>
